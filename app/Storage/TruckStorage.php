...
class TruckStorage
{
    public function add(array $data)
    {
        ...
        // account stock
        $stock_type = AccountType::where('type', 'stock')->first();
        $account_stock = $stock_type->accounts()->create([
            'account' => 'Stock ' . $data['registered'],
        ]);

        // create new Truck
        $truck = Truck::create([
            "registered" => $data['registered'], "transporter" => $data['transporter'],
            "assurance" => $data['assurance'], "visit_technique" => $data['visit_technique'],
            'account_caisse_id' => $account_caisse->id, 'account_charge_id' => $account_charge->id,
            'account_stock_id' => $account_stock->id, "creator_id" => auth()->id()
        ]);
        $this->addStock($truck);
